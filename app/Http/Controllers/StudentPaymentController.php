...
class StudentPaymentController extends Controller
{
        ...
        if(!empty($dispensation)){
            ...
            if(($dispensation->dispensation_mode == PaymentDispensations::MODE_FULL_SETUP) || ($dispensation->dispensation_mode == PaymentDispensations::MODE_REAL_PAYMENT)){
                ...
                if(count($dispensation->details) > 1){
                    ...
                    return view('student-dashboard.ppdb-billing.payment-discount-only', $data);
                }
            }
        }else{
            if($type == PaymentDispensations::DISPENSATION_TYPE_DEVELOPMENT){
            ...
            return view('student-dashboard.ppdb-billing.payment-options', $data);
        }
    }
    ...
    public function store(Request $request, PaymentDispensationsService $paymentDispensationService){
        ...
        try{
            ...
            if($request->paymentType == 'lunas'){
                ...
                return redirect()->route('bills.payment-now', ['id' =>$dispensation->id, 'type' => PaymentVirtualAccounts::VIRTUAL_ACCOUNT_FULL_STATEMENT, 'payment_type'=>$paymentType,'dispensation_type'=>$type])->with('message', 'Data berhasil disimpan!');
            }
            //Redirect Route jika sudah berhasil simpan
            return redirect()->route('bills.choise-payment',['type'=>$type])->with('message', 'Data berhasil disimpan!');
        }catch(\Exception $e){
            dd($e);
        }
    }
    ...
    public function paymentNow(Request $request, PaymentDispensationsService $paymentDispensationsService, PaymentVirtualAccountsService $paymentVirtualAccountsService){
        ...
        if($dispensation){
            ...
            return redirect()->route('bills.choise-payment',['type'=>$dispensation_type])->with('error', 'Data tidak ditemukan!');
        }
    ...
    public function paymentCancel(Request $request, PaymentVirtualAccountsService $paymentVirtualAccountsService){
        ...
        if($va_unpaid){
            if($va_unpaid->status == PaymentVirtualAccounts::STATUS_UNPAID){
                ...
                return redirect()->route('bills.choise-payment', ['type'=>$dispensation_type])->with('message', 'Pembayaran berhasil dibatalkan!');
            }else{
                return redirect()->route('bills.choise-payment', ['type'=>$dispensation_type])->with('message', 'Pembayaran sudah lunas!');
            }
    ...
    public function paymentPaidReceipt(){
        ...
        return view('student-dashboard.ppdb-billing.payment-development-receipt');
    }
    ...
    public function developmentPaymentReceipt(Request $request, PaymentDispensationsService $paymentDispensationsService)
    {
        ...
        if (!$dispensation || $dispensation->ppdb_user_id !== $user['ppdb']['id']) {
            return redirect()->route('bills.choise-payment')->with('error', 'Data tagihan tidak ditemukan.');
        }

        if ($dispensation->remaining_balance > 0) {
            return redirect()->route('bills.choise-payment')->with('error', 'Tagihan ini belum lunas.');
        }
        ...
        return view('student-dashboard.ppdb-billing.payment-development-receipt', $data);
    }
    ...
    public function changePaymentMethod(Request $request){
        ...
        if($dispensation){
            ...
            $dispensation->delete();
            return redirect()->route('bills.choise-payment', ['type' => $request->dispensation_type])->with('message', 'Cara Bayar Berhasil Batalkan!');

        }
        return redirect()->route('bills.choise-payment', ['type' => $request->dispensation_type])->with('message', 'Data Tidak Ditemukan!');
    }
    ...
    public function checkPaymentStatus(Request $request, \App\Services\OpenApi\v1\PaymentBCAService $paymentBCAService, PaymentVirtualAccountsService $paymentVirtualAccountsService)
    {
        ...
        if (!$va_unpaid) {
            return redirect()->route('bills.choise-payment', ['type' => $dispensation_type])->with('message', 'Cek status berhasil, pembayaran sudah terbayar dan terkonfirmasi.');
