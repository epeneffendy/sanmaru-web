...
class MandiriV2UniformPaymentsImport
{
    private $collection;
    private $success = [];
    private $failure = [];
    ...
    {
        try {
            if (isset($row['Txn Amount'])) $row['Txn Amount'] = Helper::stringToNumber($row['Txn Amount']);
    ...
    {
        if (isset($row['Txn Amount']) && (! in_array($row['Txn Amount'], $data['grand_totals']))) {
            $this->failure[] = '[ROW ' . $rowNumber . '] ' . $data['register_number'] . ' ' . $data['name'] . ' nominal tidak sesuai ' . $row['Txn Amount'] . ' seharusnya ' . implode(',',$data['grand_totals']);
            return null;
        }
    ...
    {
        try {
            $this->collection->put($data['register_number'] . '-' . $params['Txn Amount'], $params);
