...
use App\Rules\MultipleDateFormatRule;
...
class StudentsImport implements ToCollection, WithHeadingRow, WithChunkReading, SkipsOnError, WithBatchInserts
{
    ...
    private function storeOrUpdate($params)
    {
        ...
        if ($this->overwrite) {
            $this->studentService->updateByNis($params);
        } else {
            // If the user got soft-deleted
            if ($isUserDeleted) {
                $acc['user']->restore();
                $this->userService->resetPassword($acc['user'], $this->defaultPassword);
                $params['username'] = $acc['user']->username;
            }
            // If the student got soft-deleted
            if($isStudentDeleted) {
                $acc['student']->restore();
                $this->studentService->updateByNis($params);
            }
            else {
               $this->userService->register(User::STUDENT, $params);
            }
        }
    }
