                    ...
                    <div class="panel-body">
                        ...
                        @foreach($categories as $category)
                            <li data-category-id="{{ $category->id }}" class="list-group-item">
                                ...
                                <a href="{{ route('admin.about.index', ['aboutCategory' => $category['slug']]) }}">
                                    <strong>{{ $category->name }}</strong>
                                </a>
                                <span class="badge">{{ $category->abouts_count }}</span>
                            </li>
                        @endforeach
                        </ul>
                        
                        <div class="btn-group padding-t-10 pull-right">
                            <a href="{{ route('admin.about.category.index') }}" class="btn btn-sm btn-default">
    ...
    <script>
        $(document).ready(function () {
            function updateOrder(orderIds) {
                ...
                $.ajax({
                    url: '{{route('admin.about.category.order')}}',
