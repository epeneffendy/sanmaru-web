...
@section('content')
    @if(@$status=="edit")
        @php($action=route('admin.blog.update',array($blog['id'])))
        @php($status="Update")
        @php($status_header="Edit")
    @else
        @php($action=route('admin.blog.insert'))
        @php($status="Save")
        @php($status_header="Tambah")
    @endif
    <!-- Start Page Header -->
    <div class="page-header">
        <h1 class="title">Setting Blog</h1>
        <ol class="breadcrumb">
            <li>Konten</li>
            <li><a href="{{route('admin.blog.index')}}">Setting Blog</a></li>
                    ...
                    <div class="widget-content">
                        ...
                        <form role="form" method="POST" action="{{$action}}"  class="form-horizontal" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <div class="checkbox checkbox-success">
                                        <input id="is_unit" name="is_unit" type="checkbox" {{ (old('is_unit', @$blog->unit_id)) ? 'checked' : NULL }}>
                                        <label for="is_unit">
                                            Sebagai Unit
                                        </label>
                                    </div>
                                    
                                    <select name="unit_id" id="unit_id" class="form-control" {{ (old('is_unit', @$blog->unit_id)) ? NULL : 'style=display:none' }}>
                                        <option value="0">--- Pilih unit ---</option>
                                        @foreach($units as $key => $value)
                                            <option value="{{ $key }}" {{ old('unit_id', @$blog->unit_id) == $key ? 'selected' : NULL }}>{{ $value }}</option>
                                        @endforeach 
                                    </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="title" id="title" value="{{old('title', @$blog['title'])}}" placeholder="masukkan judul" required>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-sm-2" for="short_desc">Deskripsi:</label>
                                <div class="col-sm-10">
                                    <textarea class="form-control" name="short_desc" placeholder="masukkan deskripsi" required id="short_desc">{{old('short_desc', @$blog['short_desc'])}}</textarea>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="hidden" name="publish_date" value="{{ old('publish_date', @$blog->publish_date) }}" />
                                    <input type="text" readonly class="form-control date" name="date" id="publish_date" value="{{old('date', @$blog['pub_date'])}}" placeholder="tanggal publish" required>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-sm-2" for="content">Konten:</label>
                                <div class="col-sm-10">
                                    <textarea class="form-control" name="content" placeholder="konten" id="content">{{old('content', @$blog['html_content'])}}</textarea>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    @if (\App\Helpers\Helper::canPublishArticle())
                                        <input type="checkbox" name="published" value="1" class="custom-control-input" id="published" {{old('published', @$blog['published']) ? 'checked' : ''}}>
                                    @else
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#blog_category_id').selectpicker({liveSearch: true});
