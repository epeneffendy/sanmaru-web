        ...
        <div class="row">
            ...
            <div class="panel panel-primary">
                    ...
                    <div class="panel-body">
                        <form role="form" autocomplete="off" method="GET" action="{{route('admin.blog.index')}}">
                            ...
                            <a href="{{route('admin.blog.index')}}" class="pull-right btn btn-sm btn-warning">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body table-responsive">
                                    ...
                                    <tr>
                                        <td>
                                            <div style="display: inline-block; width: 125px; vertical-align: top;">
                                                <img src="{{ $blog->getFeaturedImageUrl() }}" style="width: 120px; height: auto;" />
                                            </div>
                                        ...
                                        <td>{{ @$blog->unit->name }}</td>
                                        <td>{{ date('j F Y H:i', strtotime($blog['publish_date'])) }}</td>
                                        <td>{!! $blog['published_label'] !!}</td>
                                        <td>
                                            <a href="{{ route('admin.blog.edit',$blog['id']) }}" class="btn btn-xs btn-default">
                                                <icon class="icon-plus"><i class="fa fa-pencil"></i></icon>
                                            </a>
                                            <a onclick="confirmDelete({{$blog['id']}})" title="Delete" class="btn btn-xs btn-danger">
                                                <i class="fa fa-trash"></i>
                                            </a>
                                            <form id="form-delete-{{$blog['id']}}" action="{{ route('admin.blog.delete',$blog['id']) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
                        ...
                        <div class="btn-group padding-t-10 pull-right">
                            <a href="{{ route('admin.blog.add') }}" class="btn btn-sm btn-success">
