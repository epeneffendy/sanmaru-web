<tr class="product-detail-row">
    ...
    <td>
        <label class="form-label price_ppdb">{{ @$price_ppdb }}</label>
        <input type="hidden" name="price_ppdb[]" value="{{ @$price_ppdb }}">
    </td>
    @endif
    <td>
    ...
    {{-- <td>
        <label class="form-label note">{{ @$note }}</label>
        <input type="hidden" class="form-control input-sm" name="note[]" style="width:15rem" value="{{ @$note }}">
    </td> --}}
    <td>
        <label class="form-label total_price">{{ @$total_price }}</label>
        <input type="hidden" class="form-control input-sm" name="total_price[]" style="width:15rem"
            value="{{ @$total_price }}">
    </td>
    @if(! @$mode == 'show')
    <td>
        <button type="button" class="btn btn-sm btn-light btn-edit-selected-product"><i
                class="fa fa-pencil"></i></button>
        <button type="button" class="btn btn-sm btn-danger btn-delete-selected-product"><i
                class="fa fa-trash"></i></button>
        <button type="button" class="btn btn-sm btn-success btn-update-selected-product" style="display: none;"><i
