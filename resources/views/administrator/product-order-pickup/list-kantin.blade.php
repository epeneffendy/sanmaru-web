...
<div class="fixed-table-head">
    <table id="datatables-product-orders-pickup" class="table display table-responsive">
        ...
        <tbody>
            @foreach($product_orders['kantin'] as $key => $value)
            <tr>
                ...
                <td>
                    ...
                    @if (!$value->isPickup())
                    <button data-toggle="modal" data-target="#resetPickupScheduleKantinModal" class="btn btn-sm btn-warning" style="margin-top: 5px">Reset</button>
                    <!-- Modal -->
                    <div id="resetPickupScheduleKantinModal" class="modal fade" role="dialog">
...
{{ $product_orders['kantin']->appends(request()->except('page'))->links() }}
