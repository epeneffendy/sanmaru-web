                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body table-responsive">
                        ...
                        <div class="button-collection">
                            <a href="{{ route('admin.teacher.export') }}" download class="btn btn-success btn-sm"><i class="fa fa-file-excel-o"></i> Export</a>
                            <a href="{{ route('admin.teacher.export', ['template-only']) }}" download class="btn btn-primary btn-sm"><i class="fa fa-save"></i> Download Template .xls</a>
                        ...
                        <div class="fixed-table-head period">
                            <table id="datatables-master-teacher" class="table display">
                                <thead>
                                <tr>
                                    <th>NIK</th>
                                ...
                                @foreach($data as $key => $value)
                                    <tr>
                                        <td>{{$value->nik}}</td>
                                        <td>{{$value->user ? $value->user->username : NULL}}</td>
                                        <td>{{$value->name}}</td>
                                        <td>{{$value->email}}</td>
                                        <td>{{$value->mobile_phone}}</td>
                                        <td>{{$value->address}}</td>
                                        <td>
                                            <a href="{{ route('admin.teacher.edit',$value->id) }}" class="btn btn-xs btn-default">
                        ...
                        <div class="btn-group padding-t-10 pull-right">
                            <a href="{{ route('admin.teacher.add') }}" class="btn btn-sm btn-success">
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form">
                        <form class="fieldset-form" method="POST" enctype="multipart/form-data" action={{ route('admin.teacher.import')}}>
                            @csrf
                            <fieldset>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#datatables-master-teacher').DataTable();
