...
<!--             <td>{{ $productOrder['nama_anak'] }}</td>
        ...
        @if (isset($merged[$key]))
            ...
            <td rowspan="{{ $rowspan }}">{{ $productOrder['status_pengambilan'] }}</td>
            <td rowspan="{{ $rowspan }}">{{ $productOrder['jadwal_pengambilan'] }}</td>
            <td rowspan="{{ $rowspan }}">{{ $productOrder['tanggal_pengambilan'] }}</td>
            @php $nomor++; @endphp
        @endif
        </tr>
    @endforeach
