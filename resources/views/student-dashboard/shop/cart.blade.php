...
@section('content')
    <div class="container container-desktop">
        ...
        {{-- <a href="{{ route('embed-product') }}" class="arrow-back"></a>
        <span>Keranjang</span> --}}
        {{-- </div> --}}
                ...
                <div class="row item">
                    <div class="item-upper">
                        ...
                        <div class="info">
                            ...
                            @else
                                <div class="info-harga">{{ \App\Helpers\PriceHelper::rupiah($detail->product_detail->price_siswa) }}</div>
                            @endif
...
<script>
    $(document).on('click', '.info-delete span', function (e) {
        ...
        swal({
            ...
            if (result) {
                $.post('{{ route('embed-product.delete-cart') }}', {
                    _token: '{{ csrf_token() }}',
    ...
    $(function () {
        ...
        var myList = {!! json_encode(\App\Models\Voucher::eligible(auth('siswa')->user()->toArray())->pluck('code')->all()) !!};
    ...
    $(document).on('click', '.btn-check-voucher', function (e) {
        e.preventDefault();
        $.post('{{ route('embed-product.post-voucher') }}', {
            _token: '{{ csrf_token() }}',
    ...
    $(document).on('click', '.btn-delete-voucher', function (e) {
        e.preventDefault();
        $.post('{{ route('embed-product.delete-voucher') }}', {
            _token: '{{ csrf_token() }}',
        }, function (data, status) {
            if (data.status === 'success') {
                voucher = {};
        ...
        swal({
            ...
            if (result) {
                ...
                $.post('{{ route('embed-product.post-cart') }}', {
                    ...
                    if (data.status === 'success') {
                        window.location.href = '{{ route('embed-product.order') }}/' + data.order;
                    } else {
                        swal(
                            'Gagal!',
    ...
    $(document).on('click', '.btn-voucher-detail', function (e) {
        ...
        $.post('{{ route('embed-product.detail-voucher') }}', {
            _token: '{{ csrf_token() }}',
            id: id,
            code: code
        }, function (data, status) {
