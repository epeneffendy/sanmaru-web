                            ...
                            <div class="row">
                                ...
                                <div class="col">
                                    <div class="pemesanan-item-info">
                                        <div class="pemesanan-item-info__title"><a href="{{ route('embed-product.order', ['id' => $order->id]) }}"><h3>Nomor invoice: {{ $order->invoice_no }}</h3></a></div>
                                        ...
                                        {{-- @if ($order->status === \App\Models\ProductOrder::STATUS_NEW_ORDER)
                                            <div class="pemesanan-item-info__button-cancel">
                                                <a href="{{ route('embed-product.order', ['id' => $order->id]) }}#konfirmasi-pembayaran"><button class="btn btn-green btn-sm-mobile">Konfirmasi bayar</button></a>
            ...
            <div class="pemesanan-item {{ $order->status === \App\Models\ProductOrder::STATUS_CANCEL ? 'cancel-order' : NULL }}">
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="pemesanan-item-info">
                                <div class="pemesanan-item-info__title"><a href="{{ route('embed-product.order', ['id' => $order->id]) }}">{{ $order->invoice_no }}</a></div>
        ...
        $(document).ready(function() {
            $('.btn-pemesanan-cancel').click(function(e) {
                ...
                swal({
                    ...
                    if (result) {
                        $.post('{{ route('embed-product.cancel-order') }}', {
                            _token: '{{ csrf_token() }}',
                            product_order_id: id
